            ...
            <div className="border-b border-dotted border-gray-500 rounded-b-3xl mb-10 sm:mb-16 w-[98vw] m-auto">
                <div className="flex justify-between bg-gradient-to-r from-cyan-200 via-blue-300 to-blue-400 rounded-t-lg mt-9 mb-5 overflow-hidden items-center px-3">
                    ...
                    <div className={`${items.length > 4? 'block': 'hidden'}`}>
                        <button onClick={nextHospitalScroll} className="w-8 bg-cyan-200 text-xl font-bold shadow-inner shadow-slate-500 active:shadow-slate-700 rounded-md sm:rounded-full sm:pb-1 mr-4">{'<'}</button>
                        <button onClick={prevHospitalScroll} className="w-8 bg-cyan-200 text-xl font-bold shadow-inner shadow-slate-500 active:shadow-slate-700 rounded-md sm:rounded-full sm:pb-1 ">{'>'}</button>
                    </div>
                </div>
                ...
                <div className="hospital-container overflow-x-scroll">
                    <div className="scroll-hospital">
                    {
                        items.map((item, idx) => {
                            return (
                                <div key={idx} className="bestHospital inline-block sm:w-64 w-[166px] shadow-[2px_2px_5px_gray] rounded-xl my-2 overflow-hidden cursor-pointer">
